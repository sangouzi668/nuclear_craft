...
public class C4Defuse {
    public static void handleDefuseProgress(BombDefuseProgressPacket packet){
        if(packet.currentTick >= 0){
            ...
            Minecraft.getInstance().gui.setOverlayMessage(Component.translatable(String.format("%.2f s / %.2f s", currentSecond, defuseSecond)), false);
        }
        else{
            Minecraft.getInstance().gui.setOverlayMessage(Component.translatable("---------------------------"), false);
        }
    }
}
